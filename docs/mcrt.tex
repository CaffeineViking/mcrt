...
\begin{document}
    ...
    \begin{figure}[ht]
        \centering
        \includegraphics[width=\linewidth]{share/new_render_low.png}
    \end{figure}
...
In some cases a higher resolution secondary \textit{photon map} is used to show caustics effects since these are very expensive to do in regular \textit{path tracing}. In this case, photons are only emitted towards specular objects in the scene and in much greater numbers compared to the regular \textit{photon map}. This enables one to get detailed caustic effects cheaply and fast.
    ...
    \section{Theory and Method} \label{sec:theory_and_method}
        ...
        \begin{align*}
            \vec{L} = \vec{o}_{sphere} - \vec{o}_{ray} \\
            t_{ca} = \vec{L} \cdot  \hat{d}_{ray} \\
        \end{align*}
...
\begin{align*}
    \hat{e}_1 = \vec{v}_2 - \vec{v}_1 \\
    \hat{e}_2 = \vec{v}_3 - \vec{v}_1
\end{align*}

the surface normal is extracted as \( \hat{n} =\hat{e}_1 \times \hat{e}_2 \), we check if the ray travels parallel with the triangle surface by \(\hat{p} = \hat{d_{ray}} \times \hat{e}_2\), and if the determinant  \(d = \hat{e}_1 \cdot \hat{p}\), \(d = 0 \), the ray does not intersect the triangle. Otherwise the intersection test continues:

\begin{align*}
    \vec{t} &= \vec{o}_{ray} - \vec{v_1} \\
    \vec{q} &= \vec{t} \times \hat{e_1} \\\\
    u &= \vec{t} \cdot \hat{p} * d^{-1} \\
    v &= \hat{d}_{ray} \cdot \vec{q} * d^{-1}
\end{align*} 
...
\begin{align*}
    ...
    v < 0 \; &\textbf{or}  \; u + v > 1 
\end{align*}
\emph{don't} hold, we have an intersection and the distance \(t\) the ray has to travel to the intersection point can be calculated as \(\vec{e_2} \cdot \vec{q} * d^{-1} \). The triangle normal is simply \(\hat{n} = \vec{e}_1 \times \vec{e}_2\), if \(\hat{n} \cdot \hat{r}_{ray} > 1\), o.w. \(\hat{n} = \vec{e}_2 \times \vec{e}_1\).
    ...
    The bounding sphere is calculated by picking the smallest and largest \textit{vertices} from the vertex set \(\mathcal{V}\)
    \begin{align*} 
        \vec{v}_{min} &= \underset{\vec{v} \in \mathcal{V}}{\text{arg\ min}}(\vec{v}) \\
        \vec{v}_{max} &= \underset{\vec{v} \in \mathcal{V}}{\text{arg\ max}}(\vec{v})
    \end{align*}
    and defining the bounding sphere as:
    \begin{align*}
        radius &= \frac{ || \vec{v}_{min} - \vec{v}_{max} ||}{2} \\
        \vec{origin} &= \frac{\vec{v}_{max} - \vec{v}_{min}}{2}
    \end{align*}
        ...
        \subsection{Surface Properties} \label{sec:surface_properties}
            ...
            \subsubsection{Oren-Nayar Model} \label{sec:oren-nayar_model}
                ...
                Surfaces which are diffuse and have rough texture can be accurately modeled with a \emph{Oren-Nayar}~\cite{oren1994generalization} \emph{reflection model}. It's based on the \emph{microfacet model} introduced by \emph{Torrance-Sparrow}~\cite{torrance1967theory}, that assumes a surface is composed of infinitesimally small, flat, Lambertian reflectors (as shown in the last section). An intuitive sketch of such a surface can be seen in \cref{fig:oren-nayar_model}, where the V-shaped microfacets are actually supposed to be infinitesimally small sized.
                ...
                Actually, experimentally, \emph{Y. Fujii} has found that his proposed approach matches regular Oren-Nayar better than the qualitative Oren-Nayar within \cite{oren1994generalization}.
        ...
        Most importance rays that are sent from the camera into the scene never hit a light source. Therefore, the direct light from the light sources to an intersection point needs to contribute to the total radiance at the point. This is computed with \emph{shadow rays}, which are sent from each intersection point to the sources of light. The shadow rays will provide light contributions to the radiance at the ray intersection points. To determine if the intersection point lies in shadow, a visbility test is performed. Transparent objects are obviously ignored by the visibility test.
    ...
    The path tracer produces visible and crisp caustics without significant extra computational cost. This result is interesting since caustics generally are computationally expensive in regular path tracing. We believe that this could be the effect of us having very simple geometry in the scene, and how the objects are placed in it. To improve the caustics in the general case, a caustic photon map can be used instead, as proposed by \textit{Jensen}~\cite{jensen1996global}. This would have the effect of speeding-up rendering while allowing for a reduced resolution of the global photon map.
    ...
    Both BRDFs in \texttt{mcrt} are for diffuse surfaces, but there are other BRDFs for specular surfaces as well, such as the \emph{Torrance-Sparrow}~\cite{torrance1967theory} and \emph{Cook-Torrance}~\cite{cook1982reflectance} models. It would be interesting to integrate these into our renderer too for completeness.

    Also, in the raytracer, all meshes are wrapped around a sphere for eliminating low-effort intersection tests. Usually, for meshes with many more triangles, it's insufficient, and causes long render times. Instead, a \emph{bounding volume hierarchy} should have been used, potentially reducing it to \(O(\log n)\) tests.

    In our photon mapping implementation, we only only support estimation of the direct light contributions. In \emph{Jensen's}~\cite{jensen1996global} original paper, indirect light is also accounted for in the \emph{global photon map}, and also has a high-resolution \emph{caustics photon map}. As we discussed before, this might have made photon mapping more useful and actually sped things up.

    Additional effects such as \emph{depth of field} and \emph{motion blur} can be approximated with a \emph{distribution raytracer} such as those presented in \emph{Cook et al.}~\cite{cook1984distributed}. The amount of additional work to integrate these effects into \texttt{mcrt} are not huge, and give nice results.

    Something which would be interesting to implement is a \emph{spectral raytracing} extension, with support for e.g. \emph{black-body radiation} like shown in \emph{Zhang et al.}~\cite{spectral2016zhang}. Instead of using RGB-channels for storing radiance, the ray wavelength could have been stored instead. Giving a more physically plausible render, such as radiance affected by the temperature, chromatic aberrations, and thin film light interference.

    Perhaps some sort of \emph{HDR} (High-Dynamic Range) technique, like \emph{tone mapping}, would solve the problem we get when our raytracer initially has with dark scenes, before reaching full convergence.

    For producing realistic scenes, realistic materials are needed, and \emph{textures} would have been a desirable feature to have. A \emph{texture mapping} technique for triangle meshes isn't that hard to implement, but special care is needed when dealing with texture filtering. Some \texttt{OpenGL} things we take for granted...

    Finally, parallelization support is achieved with \texttt{OpenMP}, but having support for additional hardware accelerators would have been desirable. \texttt{OpenMPI} would have been nice to have for speeding up rendering in computer clusters. The other more important addition which would have made a big difference is to support massively-parallel graphics hardware via APIs like \texttt{OpenCL} or \texttt{CUDA}, since the raytracing problem easily adapts to MC raytracing.

    If the reader of this report would like to add these or other features to \texttt{mcrt}, feel free to open a PR :)
