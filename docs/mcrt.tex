...
\begin{document}
    ...
    Radiosity for computer graphics builds on the concept of calculating how light is transferred between patches of the scene in an iterative process with, for each iterations we solve the equation:

\begin{equation}
B_i = E_i + \rho_i \sum_{j=1}^{n} F_{ij} B_j
\end{equation}
where \(B_i\) is the radiosity of patch \(i\), \(\rho_i\) is the \textit{reflectivity} of patch \(i\), and the sum is between each patch in the scene and the patch \(i\) with \(F_{ij} \) the formfactor (geometric term) between \(i\) and \(j\).
